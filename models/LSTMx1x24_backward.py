...
def sentences_to_indices(X, word_to_index, max_len):
    ...
    for i in range(m):
        ...
        for k in range(min(max_len,len(sentence))):
            w= sentence[len(sentence)-k-1]
...
plt.ylabel('loss')
plt.xlabel('epoch')
plt.savefig('../outputs/training_LSTM_1x24_len.png')

# save the model to json
model_json=model.to_json()
with open("model_LSTM_1_24_len.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("model_LSTM_1_24_len.h5")
